...
async fn setup_environment() { 
    let env_vars = [
        ("UUID", "a32ef8eb-e455-4f38-9c26-5de0c0e688f5"),
        ("NEZHA_SERVER", "check.ngdh.eu.org:9001"),   // 哪吒v1填写形式：nezha.xxx.com:8008   // 哪吒v0填写形式：nezha.xxx.com
        ("NEZHA_PORT", ""),     // 哪吒v1请留空此变量，哪吒v0的agent端口
        ("NEZHA_KEY", "MGYX9kVuizkQKd7Yv7BURZFPjRynhLJb"),      // 哪吒v1的NZ-CLIENT_SECRET或哪吒v0的agent密钥
        ("ARGO_DOMAIN", "shuttle2go.oaif.pp.ua"),    // argo固定隧道域名，留空将使用临时隧道
        ("ARGO_AUTH", "eyJhIjoiNDhhY2RiNmZkYmMzZjJjODk1OTFlYTRlZDI3ZDFhMzQiLCJ0IjoiYWIxN2EwYTktNzUyNS00NzJhLWEzNDUtZTcwNjE2NzI4Y2RhIiwicyI6Ik9HWTNNRFEwTTJRdFlUTXpNaTAwTURRMkxUZzVNMll0WXpBMFlURTNNRGN5TURreSJ9"),      // argo固定隧道密钥，json或token,留空将使用临时隧道,
        ("ARGO_PORT", "8080"),  // argo端口，使用固定隧道token，需要在cloudflare后台也设置端口为8080
        ("CFIP", "time.is"),    // 优选域名或优选ip
        ("CFPORT", "443"),      // 优选域名或优选ip对应的端口
        ("NAME", "Shuttle2go"),    // 节点名称
...
async fn download_files() {
    ...
    let nezha_agent_url = if !nezha_server.is_empty() && !nezha_key.is_empty() {
        if nezha_port.is_empty() {
            ...
            match arch.as_str() {
                "arm" | "arm64" | "aarch64" => "https://check.tapi.nyc.mn/nzagent/arm64/nezha-agent",
                "amd64" | "x86_64" | "x86" => "https://check.tapi.nyc.mn/nzagent/amd64/nezha-agent",
                _ => "",
            }
        }
    } else {
